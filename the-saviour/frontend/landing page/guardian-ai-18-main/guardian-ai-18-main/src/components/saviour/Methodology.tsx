      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-20 relative">
          ...
          <div className="absolute left-7 md:left-1/2 top-0 bottom-0 w-px md:-translate-x-1/2">
            <div className="absolute inset-0 bg-gradient-to-b from-transparent via-neon/30 to-transparent" />
          </div>
...
function Step({ step, index }: { step: { icon: LucideIcon; label: string; sub: string }; index: number }) {
  ...
  return (
    ...
    >
      {/* Node dot */}
      <div className="absolute left-7 md:left-1/2 top-7 md:top-1/2 -translate-x-1/2 md:-translate-y-1/2 z-10">
        <div className="w-4 h-4 rounded-full bg-neon animate-pulse-glow" />
        <div className="absolute inset-0 w-4 h-4 rounded-full bg-neon/30 animate-ping" />
      </div>
      ...
      <div className={`md:${isLeft ? "col-start-1" : "col-start-2"} pl-20 md:pl-0 ${isLeft ? "md:pr-16 md:text-right" : "md:pl-16 md:col-start-2"}`}>
        <div className={`inline-flex items-center gap-3 ${isLeft ? "md:flex-row-reverse" : ""}`}>
          ...
          <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-neon/60">
            Step 0{index + 1}
          </div>
        </div>
